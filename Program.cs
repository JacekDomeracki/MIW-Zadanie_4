//algorytm wstecznej propagacji 1 (XOR) | Jacek Domeracki | numer albumu: 173518
...
namespace Zadanie_4
{
    ...
    public class Siec_neuronowa         //warstwa nr 0 to wejścia sieci, waga nr 0 to bias neuronu
    {
        ...
        public double Oblicz_wyjscie_sieci_neuronowej(Wej_ProbkaTyp wej_probka, double beta)
        {
            ...
            return warstwy_neurony[warstwy_neurony.Length - 1][0].wartosc_wyjscia;
        }

        public void Propaguj_wstecznie_korekty(double wyj_probka, double beta, double mi)
        {
            warstwy_neurony[warstwy_neurony.Length - 1][0].korekta_wyjscia = mi * (wyj_probka - warstwy_neurony[warstwy_neurony.Length - 1][0].wartosc_wyjscia);
                                                                                                    //ta sieć ma jedno wyjście
    ...
    internal class Program
    {
        ...
        const double BETA = 1;
        const double MI = 0.3;
        const double MARGINES_BLEDU = 0.3;

        static void Main()
        {
            Console.WriteLine("ALGORYTM WSTECZNEJ PROPAGACJI ( ZADANIE 1 )");
            Console.WriteLine();
            ...
            KeyValuePair<Wej_ProbkaTyp, double> probka_rob;
            ...
            int n_pr = 0;
            double wartosc_wyjscia_SN;
            double nowa_wartosc_wyjscia_SN;
            double blizej_do_probki_wyj = 0;

            for (int i = 0; i < ILE_ITERACJE; i++)
            {
                if (i < ILE_WID_GD || i >= ILE_ITERACJE - ILE_WID_GD) Console.WriteLine("-->  EPOKA NR : {0}", i + 1);

                for (int j = 0; j < Probki_funkcji_xor.Count; j++)
                {
                ...
                for (int j = 0; j < Probki_funkcji_xor.Count; j++)
                {
                    ...
                    probka_rob = Probki_funkcji_xor.OrderBy(x => x.Key).ElementAt(n_pr);
                    ...
                    blizej_do_probki_wyj = Math.Abs(probka_rob.Value - wartosc_wyjscia_SN) - Math.Abs(probka_rob.Value - nowa_wartosc_wyjscia_SN);
                    ...
                    if (i < ILE_WID_GD || i >= ILE_ITERACJE - ILE_WID_GD)
                        Console.WriteLine("------>  PRÓBKA NR : {0}  |  NOWE WYJŚCIE : {1,8:F6}  |  UBYTEK BŁĘDU : {2,12:F10}  |  NOWY BŁĄD: {3,9:F6}",
                            n_pr + 1, nowa_wartosc_wyjscia_SN, blizej_do_probki_wyj, probka_rob.Value - nowa_wartosc_wyjscia_SN);
                    if (blizej_do_probki_wyj < 0)     //test 0.00001
                        break;
                }
                if (i < ILE_WID_GD || i >= ILE_ITERACJE - ILE_WID_GD) Console.WriteLine();
                if (blizej_do_probki_wyj < 0)     //test 0.00001
                {
                    Console.WriteLine("/\\/\\------>  EPOKA NR : {0}  |  PRÓBKA NR : {1}  |  NEGATYWNA WERYFIKACJA (PRZYROST BŁĘDU) : {2,12:F10}",
                        i + 1, n_pr + 1, blizej_do_probki_wyj);
                    Console.WriteLine();
                    break;
                }
            }
            ...
            {
                n_pr = 0;
                foreach (var probka in Probki_funkcji_xor)
                {
                    wartosc_wyjscia_SN = SN.Oblicz_wyjscie_sieci_neuronowej(probka.Key, BETA);
                    if (Math.Abs(probka.Value - wartosc_wyjscia_SN) >= MARGINES_BLEDU)
                    {
                        n_pr++;
                    }
                }
